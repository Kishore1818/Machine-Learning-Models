...
# best_model, results, trained_models = run_pipeline('shipdata.csv')

# # Or run individual steps if needed
# df = load_and_preprocess_data('shipdata.csv')
# df_featured = feature_engineering(df)
# X, y, encoders = prepare_features(df_featured)
# results, models, importance, scaler, X_train, X_test, y_train, y_test = train_models(X, y)
# (or)
# best_model, results, trained_models = run_pipeline('my_data.csv')
# (or)
# DATA_FILE = 'my_shipping_data.csv'
# best_model, results, trained_models = run_pipeline(DATA_FILE)
...
def train_models(X, y):
    ...
    return results_df, trained_models, feature_importance, scaler, X_train, X_test, y_train, y_test

def get_model_comparison(results_df):
    """
    ...
    # Sort by R2 score
    comparison = results_df.sort_values('R2_Score', ascending=False)
...
def save_best_model(best_model_name, trained_models, scaler, encoders, feature_importance):
    """
    ...
    if best_model_name in feature_importance:
        joblib.dump(feature_importance[best_model_name], 'feature_importance.pkl')
    
    print("✅ Model and preprocessing components saved successfully!")
    
    return best_model

def create_visualizations(results_df, best_model_name, trained_models, scaler, feature_importance, X_train, X_test, y_train, y_test):
    """
    ...
    # 1. Model Comparison
    plt.subplot(2, 3, 1)
    comparison_sorted = results_df.sort_values('R2_Score', ascending=True)
    ...
    # 2. RMSE vs MAE
    plt.subplot(2, 3, 2)
    scatter = plt.scatter(results_df['RMSE'], results_df['MAE'], 
                        c=results_df['R2_Score'], cmap='viridis', s=100, alpha=0.7)
    ...
    plt.subplot(2, 3, 6)
    plt.hist(y_test, bins=30, alpha=0.7, color='purple', label='Actual')
    ...
    plt.errorbar(range(len(results_df)), cv_means, yerr=cv_stds, 
...
def run_pipeline(csv_file_path='shipdata.csv'):
    ...
    # Train models
    results, trained_models, feature_importance, scaler, X_train, X_test, y_train, y_test = train_models(X, y)
    
    # Get model comparison
    best_model_name, comparison = get_model_comparison(results)
    
    # Save best model
    best_model = save_best_model(best_model_name, trained_models, scaler, encoders, feature_importance)
    
    # Create visualizations
    create_visualizations(results, best_model_name, trained_models, scaler, feature_importance, 
                         X_train, X_test, y_train, y_test)
    
    return best_model, results, trained_models
